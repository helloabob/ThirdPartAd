package
{
	...
	public class BaiduUnionAdPlugin extends Sprite
	{
		...
		public function BaiduUnionAdPlugin(){
		...
		private function loadBdAd(e:Event):void{
			...
			loader = new Loader(); 
			addEvent();
			loader.load(new URLRequest("http://cpro.baidu.com/cpro/ui/baiduLoader_as3.swf")); 
		...
		private function onBaiduAdInit(e : Event):void { 
			//标签云Loader加载完成后执行 
			DebugTip.instance.log("-----baidu_union_ad_load_complete------");
...
//			this.addChild(adBack);
			...
			param.cpro_client="78050048_tp_cpr";
			param.cpro_url="www.kankanews.com";
			param.cpro_template="baiduxml_tiepian_400_300";
			param.cpro_w=init_width;
			param.cpro_h=init_height;
			(loader.content as Object)["requestAdData"](baiduAdComplete, param);
		}
		
		private function baiduAdComplete():void{
			DebugTip.instance.log("------baidu_union_show_start-------");
		...
		public function setSize(w:Number, h:Number):void
		{
			...
			DebugTip.instance.log("-----baidu_union_set_size_w_h------"+w+","+h);
			if(!loader.content){
				return;
			}
//			drawBack(w, h);
		...
		public function closeAd():void{
			DebugTip.instance.log("-----baidu_union_ad_show_end------");
