package
{
	...
	public class BaiduHongAdPlugin extends Sprite
	{
		...
		public function BaiduHongAdPlugin(){
			
			Security.allowDomain("dn.baidu.com");
			if (stage){
				loadBdAd(null);
			}
			else{
		...
		private function loadBdAd(e:Event):void{
			...
			DebugTip.instance.log("916a-----loadBdHongAd()-----");
			DebugTip.instance.log("-----remove-----");
			
			loader = new Loader(); 
			addEvent();
			loader.load(new URLRequest("http://dn.baidu.com/flash/loader_as3.swf")); 
		...
		private function onBaiduAdInit(e : Event):void { 
			//标签云Loader加载完成后执行 
			DebugTip.instance.log("-----baidu_hong_ad_load_complete------");
...
//			this.addChild(adBack);
			...
			param.dan_w=init_width;
			param.dan_h=init_height;
			param.dan_id=425807;
			(loader.content as Object)["requestAdData"](requestedFunc, finishedFunc, param);
		}
		...
		private function requestedFunc():void{
			DebugTip.instance.log("------baidu_hong_show_start-------");
		...
		public function setSize(w:Number, h:Number):void
		{
			...
			DebugTip.instance.log("-----baidu_hong_set_size_w_h------"+w+","+h);
			if(!loader.content){
				return;
			}
//			drawBack(w, h);
		...
		public function closeAd():void{
			DebugTip.instance.log("-----baidu_hong_ad_show_end------");
